...
def plot_result(n,entities):    
  ...
  #plt.show()  
  plt.savefig('work/adder_comparison.svg', bbox_inches='tight')
...
def main(argv):
    ...
    area.resize((len(argv),len(n)))   
    power.resize((len(argv),len(n)))   
    fmax.resize((len(argv),len(n))) 

    printProgressBar(0, l, prefix = 'Progress:', suffix = 'Complete', length = 50)

    for i, entity in enumerate(argv):

      vhdfile = open('adder.vhd','r')
      lines = vhdfile.readlines()
      vhdfile.close()
      vhdfile = open('adder.vhd','w')
      for line_i,line in enumerate(lines):
        if 'adder_inst: entity work' in line:
          lines[line_i] = '  adder_inst: entity work.' + entity + '\n'
      vhdfile.writelines(lines)
      vhdfile.close()
   
      for j,width in enumerate(n):
        lines = os.popen(f'analyze.py adder -p N={width}').read().split('\n') 
        parse(lines, i, j, T)
